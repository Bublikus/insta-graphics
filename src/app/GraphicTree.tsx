...
function collectExpandedGroupIds(
  nodes: GraphicNode[],
  activeId: string,
        ...
        <li key={node.id} className={`graphic-group-item ${isExpanded ? 'is-open' : 'is-closed'}`}>
          ...
          {isExpanded ? (
            <ul id={panelId} className="graphic-sub-list">
              {renderTreeNodes(node.children, activeId, expandedGroupIds, onToggleGroup, onNavigate, depth + 1)}
            </ul>
          ) : null}
        </li>
      )
...
export function GraphicTree({ graphicTree, activeId, onNavigate }: GraphicTreeProps) {
  ...
  const [expandedGroupIds, setExpandedGroupIds] = useState<Set<string>>(
    () => new Set(activeExpandedGroupIds),
  )
  ...
  return (
    <ul className="graphic-link-list">
      {renderTreeNodes(graphicTree, activeId, expandedGroupIds, onToggleGroup, onNavigate)}
    </ul>
  )
}
